...
//     提供一个多线程安全的数据库实例
        [queue inDatabase:^(FMDatabase *db) {
            BOOL flag = [db executeUpdate:@"create table if not exists t_collectFMDB (id integer primary key autoincrement,count integer,currentDate text,weightData text)"];
            if (flag) {
                NSLog(@"创建表 success");
            } else {
                NSLog(@"创建表 failed");
...
- (void)queryBtn {
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        while ([result next]) {
            ...
            NSString *weightData = [result stringForColumn:@"weightData"];
